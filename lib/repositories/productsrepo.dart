...
class ProductsRepo extends Repository {
  ...
  Future<List<OrderModel>> getMyOrdersList({ScreenCallback listener}) async {
    listener.showProgress();
    String url = UrlProvider.getOrdersUrl();
    print(url);
    ResponseStatus responseStatus = await networkManager.get(url: url);
    listener.hideProgress();
    if (responseStatus != null) {
      if (responseStatus.getError() == NetworkConstants.OK) {
        List<OrderModel> orderDetailsList =
            responseParser.getMyOrdersList(responseStatus.getData());
        return orderDetailsList;
      } else {
        //error = 1
        listener.onError(responseStatus.getMessage());
